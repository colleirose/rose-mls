...
use openssl::{bn::BigNumContext, ec::{EcGroup, EcKey, EcPoint}, error::ErrorStack, hash::MessageDigest, nid::Nid, pkey::Id};

#[cfg(feature = "x509")]
use openssl::pkey::{PKey, Private, Public};

use thiserror::Error;
use crate::{ec::{curve_to_id, ec_generate, nist_curve_id, private_key_bytes_to_public, EcError, EcPrivateKey, EcPublicKey}, MlsCryptoError};
...
pub enum EcSignerError {
    ...
    #[error(transparent)]
    EcError(#[from] MlsCryptoError),
    #[error("invalid signature")]
    InvalidSignature,
    #[error(transparent)]
    SignerError(#[from] EcError)
}
...
fn pub_key_from_uncompressed_nist(bytes: &[u8], nid: Nid) -> Result<PKey<Public>, ErrorStack> {
...
fn pub_key_from_uncompressed_non_nist(bytes: &[u8], id: Id) -> Result<PKey<Public>, ErrorStack> {
    PKey::public_key_from_raw_bytes(bytes, id)
}

fn pub_key_from_uncompressed(bytes: &[u8], curve: Curve) -> Result<PKey<Public>, MlsCryptoError> {
    let pubkey = if let Some(nist_id) = nist_curve_id(curve) {
        pub_key_from_uncompressed_nist(bytes, Nid::from_raw(nist_id))
    } else {
...
impl EcSigner {
    ...
    pub fn signature_key_derive_public(
        ...
        Ok(private_key_bytes_to_public(self.0, secret_key)?.into())
    }
    ...
    pub(crate) fn pkey_from_secret_key(
        ...
        PKey::private_key_from_raw_bytes(key, curve_to_id(self.0)?).map_err(Into::into)
    }
    ...
    pub(crate) fn pkey_from_public_key(
        ...
        PKey::public_key_from_raw_bytes(key, curve_to_id(self.0)?).map_err(Into::into)
    }
    ...
    pub fn sign(
        ...
        let secret_key = PKey::private_key_from_raw_bytes(secret_key, curve_to_id(self.0)?)?;
