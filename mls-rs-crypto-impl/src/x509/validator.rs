...
use crate::{check_non_null, check_res, MlsCryptoError};
...
impl CertificateValidator {
    pub fn new_der(ca_certs: &[DerCertificate]) -> Result<Self, MlsCryptoError> {
        let ca_certs = certs_to_stack(ca_certs)?;
        Ok(Self { ca_certs })
    }

    pub fn new(ca_certs: Vec<Certificate>) -> Result<Self, MlsCryptoError> {
    ...
    pub fn validate(
        ...
        let leaf = chain
            .leaf()
            .ok_or(MlsCryptoError::CryptoError)
        ...
        unsafe {
            ...
            if validation_result != 1 || validation_code != X509_V_OK {
                ...
                return Err(MlsCryptoError::CertValidationFailure(reason));
            }
...
fn certs_to_stack(certs: &[DerCertificate]) -> Result<Stack<Certificate>, MlsCryptoError> {
    let stack = certs.iter().try_fold(Stack::new()?, |mut stack, cert| {
        let cert = Certificate::try_from(cert)?;
        stack.push(cert);
        Ok::<_, MlsCryptoError>(stack)
    })?;
...
mod tests {
    ...
    fn will_fail_on_empty_chain() {
        ...
        assert_matches!(res, Err(MlsCryptoError::CryptoError));
    }
    ...
    fn will_fail_on_invalid_chain() {
        ...
        assert_matches!(res, Err(MlsCryptoError::CertValidationFailure(_)));
    }
