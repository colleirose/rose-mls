...
use crate::{check_int_return, check_non_null, check_non_null_const, check_res, MlsCryptoError};
...
impl X509Name {
    pub fn new() -> Result<Self, MlsCryptoError> {
        unsafe { check_non_null(X509_NAME_new()).map(Self) }
    }

    pub fn add_entry(&mut self, component: &SubjectComponent) -> Result<(), MlsCryptoError> {
    ...
    pub fn new_components(components: &[SubjectComponent]) -> Result<Self, MlsCryptoError> {
        components
            .iter()
    ...
    #[cfg(test)]
    pub fn to_der(&self) -> Result<Vec<u8>, MlsCryptoError> {
...
impl Asn1String {
    pub fn new(string_type: i32) -> Result<Self, MlsCryptoError> {
        unsafe { check_non_null(ASN1_STRING_type_new(string_type)).map(Self) }
    }

    pub fn new_value(string_type: i32, value: &[u8]) -> Result<Self, MlsCryptoError> {
    ...
    pub fn set_value(&mut self, value: &[u8]) -> Result<(), MlsCryptoError> {
        unsafe {
...
impl GeneralName {
    unsafe fn new() -> Result<Self, MlsCryptoError> {
        check_non_null(GENERAL_NAME_new()).map(Self)
    }

    fn new_value<T: Into<*mut c_void>>(name_type: i32, value: T) -> Result<Self, MlsCryptoError> {
        unsafe {
            let name = Self::new()?;
    ...
    pub fn subject_alt_name(&self) -> Result<SubjectAltName, MlsCryptoError> {
        unsafe {
            ...
            match name_type {
                ...
                GEN_RID => Err(MlsCryptoError::CryptoError),
                GEN_IPADD => Ok(SubjectAltName::Ip(asn1_to_ip(value.cast())?)),
                _ => Err(MlsCryptoError::CryptoError),
            }
        }
    }

    pub fn email(addr: &str) -> Result<Self, MlsCryptoError> {
        Self::new_value(
            GEN_EMAIL,
    ...
    pub fn uri(uri: &str) -> Result<Self, MlsCryptoError> {
        Self::new_value(
            GEN_URI,
    ...
    pub fn dns(dns: &str) -> Result<Self, MlsCryptoError> {
        Self::new_value(
            GEN_DNS,
    ...
    pub fn ip(ip: &str) -> Result<Self, MlsCryptoError> {
        let ip = ip
            .parse::<IpAddr>()
            .map_err(|_| MlsCryptoError::CryptoError)?;
...
impl X509Extension {
    pub fn subject_alt_name(alt_names: &[SubjectAltName]) -> Result<Self, MlsCryptoError> {
        let stack = alt_names
            .iter()
            .try_fold(Stack::new()?, |mut names, name| {
                let general_name = match name {
                    ...
                    SubjectAltName::Rid(_) => Err(MlsCryptoError::CryptoError),
                    SubjectAltName::Ip(ip) => GeneralName::ip(ip),
                }?;

                names.push(general_name);

                Ok::<_, MlsCryptoError>(names)
            })?;
    ...
    pub fn key_usage(critical: bool, usages: &[KeyUsage]) -> Result<Self, MlsCryptoError> {
    ...
    pub fn to_string(&self) -> Result<String, MlsCryptoError> {
        unsafe {
            ...
            String::from_utf8(out_buffer).map_err(|_| MlsCryptoError::CryptoError)
        }
    }
}
...
) -> Result<X509Extension, MlsCryptoError> {
    let c_string = CString::new(string).map_err(|_| MlsCryptoError::CryptoError)?;
...
unsafe fn asn1_to_string(value: *mut ASN1_STRING) -> Result<String, MlsCryptoError> {
    unsafe {
        ...
        String::from_utf8(slice.to_vec()).map_err(|_| MlsCryptoError::CryptoError)
    }
}

unsafe fn asn1_to_ip(value: *mut ASN1_STRING) -> Result<String, MlsCryptoError> {
    unsafe {
        let ptr = check_non_null_const(ASN1_STRING_get0_data(value))?;
        let len = check_int_return(ASN1_STRING_length(value))?;
...
pub(super) unsafe fn components_from_name(
    name: *mut X509_NAME,
) -> Result<Vec<SubjectComponent>, MlsCryptoError> {
    (0..X509_NAME_entry_count(name)).try_fold(Vec::new(), |mut components, i| {
        ...
        let data = String::from_utf8(slice.to_vec()).map_err(|_| MlsCryptoError::CryptoError)?;
