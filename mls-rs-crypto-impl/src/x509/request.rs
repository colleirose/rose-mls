...
use crate::{check_int_return, check_non_null, check_res, ecdsa::AwsLcEcdsa, MlsCryptoError};
...
impl X509Request {
    ...
    pub fn set_version(&mut self, version: X509RequestVersion) -> Result<(), MlsCryptoError> {
        unsafe { check_res(X509_REQ_set_version(self.0, version as c_long)) }
    }

    pub fn set_subject(&mut self, subject: X509Name) -> Result<(), MlsCryptoError> {
        unsafe { check_res(X509_REQ_set_subject_name(self.0, subject.0)) }
    }

    pub fn add_extensions(&mut self, ext: Stack<X509Extension>) -> Result<(), MlsCryptoError> {
        unsafe { check_res(X509_REQ_add_extensions(self.0, ext.inner.cast())) }
    }

    fn set_public_key(&mut self, key: EvpPkey) -> Result<(), MlsCryptoError> {
