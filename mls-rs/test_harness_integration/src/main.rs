...
type TestClientConfig = WithIdentityProvider<
    ...
    WithCryptoProvider<MainCryptoProvider, WithMlsRules<TestMlsRules, BaseInMemoryConfig>>,
>;

#[cfg(feature = "by_ref_proposal")]
type TestExternalClientConfig = mls_rs::external_client::builder::WithIdentityProvider<
    BasicIdentityProvider,
    mls_rs::external_client::builder::WithCryptoProvider<MainCryptoProvider, ExternalBaseConfig>,
>;
...
async fn create_client(cipher_suite: u16, identity: &[u8]) -> Result<ClientDetails, Status> {
    ...
    let provider = MainCryptoProvider::new()
